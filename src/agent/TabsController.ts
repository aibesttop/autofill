 ...
 * - lives in the agent env (extension page or content script)
 * - no chrome apis directly; calls SW for tab operations
 */
    ...
    const tabChangeHandler = (message: any): void => {
      ...
      if (message.action === 'created') {
        ...
        if (targetTab) {
          ...
          targetTab.status = tab.status as TabMeta['status'];
        }
      }
    };
  ...
  async openNewTab(url: string): Promise<string> {
    ...
    if (!this.tabGroupId) {
      ...
      if (!groupResult.success) {
        throw new Error(`Failed to create tab group: ${groupResult.error}`);
      }

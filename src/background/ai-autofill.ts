...
function buildFieldMappingPrompt(request: LLMFieldMappingRequest): string {
  return [
    ...
    '- Use contactEmail only when it is present in the website profile; otherwise do not invent email, phone, password, login, payment, captcha, or verification values.',
    '- For comment or reply fields, prefer submissionComment when it is present in the website profile.',
...
function buildPageActionPrompt(request: LLMPageAutofillPlanRequest): string {
  return [
    ...
    '- Use contactName, contactEmail, submissionWebsiteUrl, and submissionComment for matching name, email, website, comment, or reply fields when present.',
    '- Do not wrap the JSON in markdown.',
    '',
    `Page title: ${request.pageTitle || 'Untitled page'}`,
    `Page URL: ${request.pageUrl}`,

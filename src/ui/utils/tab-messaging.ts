...
function normalizeTabMessagingError(error: unknown, url?: string): Error {
  ...
  if (isMissingPermissionError(error)) {
    return new Error('The extension does not currently have access to this site. Reload the extension and refresh the page.');
  }
...
export async function sendMessageToTab<TResponse>(
  ...
  try {
    ...
    if (!isMissingReceiverError(error)) {
      throw normalizeTabMessagingError(error, tab.url);
    }

    try {
      await injectContentScript(tab.id);
      return await sendMessageWithRetry<TResponse>(tab.id, message);
    } catch (retryError) {
      throw normalizeTabMessagingError(retryError, tab.url);
    }
  }
}
